...
class AddressBook {
  ...
  findContactByName(name: string): Contact | undefined {
    return this.contacts.find((contact: Contact): boolean => contact.name === name);
  }
  ...
  sortByName(): void {
    this.contacts.sort((a: Contact, b: Contact): number => a.name.localeCompare(b.name));
  }
  ...
  searchContacts(searchTerm: string): Contact[] {
    ...
    return this.contacts.filter((contact: Contact): boolean =>
...
searchResults.forEach((contact: Contact): void => console.log(`  - ${contact.name}`));